...
function calculateGrade(Type $type)
{
    ...
    $nilaiKog = round(($formData["tugas-kognitif"] + $formData["ulangan-kognitif"]) + (2 * $formData["uts"]) + (2 * $formData["uas"])) / 6;
    $nilaiKet =  round(($formData["tugas-keterampilan"] + $formData["ulangan-keterampilan"]) + (2 * $formData["uts"]) + (2 * $formData["uas"])) / 6;
    $nilaiAkhir = round(($nilaiKog + $nilaiKet) / 2);
