...
if ($method === "POST") {
    ...
    switch ($batasDaya) {
        case BatasDaya::EMPAT_RATUS_LIMA_PULUH->value:
            ...
            // Menampilkan hasil
            echo "Pemakaian kWh: " . $pemakaianKwh . " kWh<br>";
            echo "Biaya Beban: Rp " . number_format($biayaBeban, 0, ',', '.') . "<br>";
            echo "Biaya Pemakaian Blok I: Rp " . number_format($biayaPemakaianBlokSatu_450, 0, ',', '.') . "<br>";
            echo "Biaya Pemakaian Blok II: Rp " . number_format($biayaPemakaianBlokDua_450, 0, ',', '.') . "<br>";
            echo "Biaya Pemakaian Blok III: Rp " . number_format($biayaPemakaianBlokTiga_450, 0, ',', '.') . "<br>";
            echo "Total Biaya Pemakaian Semua Blok: Rp " . number_format($totalBiayaSemuaBlok, 0, ',', '.') . "<br>";
            echo "Pajak Penerangan Jalan (PPJ): Rp " . number_format($pajak, 0, ',', '.') . "<br>";
            echo "Total yang harus dibayar: Rp " . number_format($totalAkhir, 0, ',', '.') . "<br>";
            break;
