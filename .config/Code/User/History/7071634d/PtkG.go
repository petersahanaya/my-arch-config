...
func BuyTickets() {
	if isAuthenticated(&user) {
		...
		fmt.Println("What film you want to watch?")

		timeout(2)

		movieTitle := new(string)
		input.Prompt("Movie name\t: ", movieName)

		foundMovies := find.Find(&Movies, func(title) {
			return strings.Contains(title, *movieTitle)
		})
	}
}
