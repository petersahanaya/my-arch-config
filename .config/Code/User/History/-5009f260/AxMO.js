...
const cekKamar = (kodeKamar) => {
    switch(kodeKamar) {
        case EKONOMI.nama: 
            const total = validateBiaya(EKONOMI.biaya)

            container.insertAdjacentElement("beforeend")
            break;
        case VIP.nama:
            const total =validateBiaya(VIP.biaya)
            container.insertAdjacentElement("beforeend", domTotal())
            break;
        case VVIP.nama:
           const total = validateBiaya(VVIP.biaya)
            container.insertAdjacentElement("beforeend")
            break;
    }
}
